...
const SingleProduct = ({ product }) => {
  ...
  return (
    <div className=" my-3">
      <Card
        className={`mx-2 rounded-3 bg-white shadow-sm ${styles.content}`}
        data-aos="fade-up"
      >
        <div className="mx-2 my-1 d-flex justify-content-between align-items-center">
          <small
            className={`px-2 py-1 fw-light rounded-pill text-white ${styles.discount}`}
          >
            {price > 15000 ? "20% " : "10% "}
            off
          </small>
        ...
        <div className={`${styles.contentDetails} ${styles.fadeInBottom}`}>
          <Button
            className={`my-3 me-2 fw-bold border-0 rounded-pill ${styles.myButton}`}
            variant="secondary"
          >
            <FaCartArrowDown className="mb-1 me-2 " />
          </Button>
          <Button
