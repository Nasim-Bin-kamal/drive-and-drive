...
const ProductDetails = () => {
    ...
    const { carName, price, image, company, model, engineCapacity, power, breaking, category } = product || {}
                ...
                <Row className='d-flex justify-content-center align-items-center' xs={1} md={2} lg={2}>
                    ...
                    <Col>
                        <div className='mx-auto p-3'>
                            <h2>{carName}</h2>
                            <p><span className='text-secondary'>Brand:</span> {company}</p>
                            <p><span className='text-secondary'>Rated:</span> 5.0</p>
                            <h4 className={styles.price}>${price}</h4>
                            <small className='text-secondary d-block'>Stock Available</small>
                            <Button
                                onClick={handleModalShow}
                                className={`my-3 fw-bold border-0 rounded-pill ${styles.myButton}`} variant="dark"><GiConfirmed className='mb-1 me-2' />Order Now</Button>
                        </div>
                    </Col>
                </Row>
                <hr />
